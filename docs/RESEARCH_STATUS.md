...
**CURSOR-ASSIST RESEARCH ROUND (4 parallel workers, 2026-06-26 ~16:00)**:
Cursor agent ran 4 parallel background research workers to resolve the remaining OPEN questions tied to each cursor-task. Each worker owned disjoint task files (no edit conflicts), read the specs, did focused 2026 web research, and appended a "## Deep Research Resolutions (sibling worker, 2026-06-26)" section to its task file(s). RESEARCH_STATUS + roadmap-and-open-questions left untouched by workers (parent logged here).

- **Core mechanics (tasks 002/003/004)** — DONE. Fractional-T accumulator + ascending bar apply on integer crossings (no skipped bars under FF); Pause-wins-by-state simultaneity (overridden FF cost suspended); multi-FF R=max with each payer paying own tier; anonymized influence (preserves bluffing); server-authoritative R/T + optimistic own-input only; max R=5; NO intra-bar interpolation (daily bar atomic). IP: realized-profit-only grants, never negative, clamp at soft cap (no decay), cooldown+cost(+optional GCD) anti-spam; sabotage always-notify victim-only timestamped, player-targeted (not instrument-specific) MVP, NO purge ability; deterministic Widen stacking; Fake-News lifetime; Peek = current-T single headline. Orders: OHLC high/low triggers, pessimistic stop-first ambiguity rule, exit-first canonical (instrument_id, order_id) ordering, fixed ~7bps base spread (widen stacks), percent-of-capital sizing (no margin calls), proportional partial closes feed +10% IP, market = next-open + spread. All spec numbers preserved; determinism checklists added.
- **Backend/determinism (tasks 001/005)** — DONE. 30Hz fixed sim tick integrating wall-time; 12–15Hz coalesced delta broadcasts + immediate events + ~2s keyframes; pure server-follow for contested clock (optimistic own-input UI only; client interpolates toward server T, never extrapolates); single-process asyncio one-task-per-room RoomManager (Redis deferred); exact TB resolver. Determinism contract: canonical state = pure fn of (arena, sorted action log); Decimal + SHA-256 over sorted-key JSON (never builtin hash()); GGPO-adapted SyncTest (bit-identical checkpoint+final hashes); verify_replay auto-flags mismatch; lag-injection arrival-jitter + adversarial Pause/FF race tests; Hypothesis property-based fuzz; periodic in-match checksums.
- **Data pipeline (task 006)** — DONE (parent completed after the delegated worker hit resource_exhausted; no work lost). Persist RAW OHLC, normalize at load (P[0]=100 on close, same factor to O/H/L/C to preserve fill geometry); Parquet v2 + ZSTD lvl5 + write_statistics, partition raw bulk by instrument/year, one file per tiny arena; volume nullable + excluded from norm/hash; pandas_market_calendars valid_days + inner-join on common sessions (skip, never interpolate); mystery start offset baked at curation; content_hash = SHA-256 over canonical sorted JSON of normalized 4dp prices; MVP arena selection = manual curation of 12–16 regime-tagged arenas + balanced-random over buckets (HMM+K-Means noted as upgrade); deterministic rolling-stat regime heuristic. 2026 source status verified: yfinance fragile/rate-limited (use curl_cffi, chunk, cache, one-time fetch), Stooq still active bulk no-signup (primary), FRED fallback for US closes.
- **Frontend/voice (tasks 007/008)** — DONE. LW Charts v5 API correction: series.setMarkers() removed → use createSeriesMarkers(series,…); addSeries(CandlestickSeries,…) replaces addCandlestickSeries. Scrub = local read-only pan/zoom + "Return to Live" (never mutates server T); ease R ~150–250ms, snap discrete bar reveals, freeze on pause; Current-T HUD chip; equity own pane; diff overlays via marker primitive + Map<orderId, IPriceLine>; guarded master/slave; useReplayController rAF hook scaled by R (clears on R<=0); delta shapes + Zustand mapping; synthetic "Sim Day N" axis. Voice: managed-provider TURN for MVP behind /ice-servers (Coturn at scale), HMAC-SHA1 ephemeral creds, ~15–25% relay (TURN mandatory); <150ms w/ Opus FEC ~24–32kbps; MDN perfect-negotiation + trickle ICE over task-005 WS; getStats fallback/ICE-restart; mic-denial → text; PTT/mute via track.enabled; @ricky0123/vad-web indicators; psych event→taunt mapping; VoicePanel beside TextChat; recording deferred.

Net: every Tier-1/2/3 OQ tied to the 8 tasks now has a concrete, spec-faithful, cited resolution appended in-place; tasks are implementation-ready.
